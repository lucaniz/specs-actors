...
func (a Actor) CompactPartitions(rt Runtime, params *CompactPartitionsParams) *adt.EmptyValue {
	...
	rt.State().Transaction(&st, func() interface{} {
		...
		// TODO: enforce this for the particular deadline only?
		if havePendingEarlyTerminations(rt, &st) {
			rt.Abortf(exitcode.ErrForbidden, "cannot compact partitions with outstanding early terminations")
		}
